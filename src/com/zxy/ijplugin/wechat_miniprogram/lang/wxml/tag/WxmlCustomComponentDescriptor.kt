...
class WxmlCustomComponentDescriptor(private val element: JsonProperty) : WXMLBasicElementDescriptor() {
  ...
  override fun getAttributeDescriptor(attributeName: String, context: XmlTag?): XmlAttributeDescriptor {
    var descriptor = super.getAttributeDescriptor(attributeName, context)
    if (descriptor == null) {
      descriptor = WXMLUtils.getCustomComponentAttributeDescriptors(this).find {
        var attrName = attributeName
        // abc-edf也是合法的属性名，实际上定义的时候是没有-的
        if (attrName.contains("-")) {
          attrName = attrName.replace("-", "")
        }
        // 不区分大小写
        it.name.equals(attrName, true)
      }
    }
    return descriptor ?: AnyXmlAttributeDescriptor(attributeName)
  }
}
