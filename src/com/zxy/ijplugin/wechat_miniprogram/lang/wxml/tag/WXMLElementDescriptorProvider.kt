...
class WXMLElementDescriptorProvider : XmlElementDescriptorProvider {
  ...
  private fun findCustomComponentJsonProperty(xmlTag: XmlTag): JsonProperty? {
    ...
    val jsonFile = RelateFileHolder.JSON.findFile(wxmlPsiFile.originalFile) as? JsonFile

    if (jsonFile == null) {
      // 未找到json文件,可能是模板wxml文件，使用app.json中的配置
      return AppJsonUtils.getUsingComponentItems(xmlTag.project).find { it.name == tagName }
    }

    // 找到usingComponents的配置
    val usingComponentItems =
      ComponentJsonUtils.getUsingComponentItems(jsonFile) +
        AppJsonUtils.getUsingComponentItems(xmlTag.project) +
        ComponentJsonUtils.getComponentGenericsItems(jsonFile)
