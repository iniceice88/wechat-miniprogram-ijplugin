...
class WXMLTagNameProvider : XmlTagNameProvider {
  override fun addTagNameVariants(
    ...
    if (tag.language != WXMLLanguage.INSTANCE) return
    ...
    if (currentJsonPsiFile === null) return
    if (currentJsonPsiFile !is JsonFile) return
    ...
    // 从app.json 和 相关的json文件中搜集usingComponents
    ...
    val usingComponentMap = usingComponentItems.associateBy({ jsonProperty ->
    ...
    elements.addAll(jsonFiles.mapNotNull { jsonFile ->
      ...
      val componentPath =
        (jsonFile.virtualFile.getPathRelativeToRootRemoveExt(jsonFile.project) ?: return@mapNotNull null)
