...
import com.zxy.ijplugin.wechat_miniprogram.extensions.withPriority
...
class WXMLAttributeNameCompletionProvider : CompletionProvider<CompletionParameters>() {
  ...
  override fun addCompletions(
    ...
    if (reference is XmlAttributeReference) {
      ...
      val xmlAttributes = xmlTag.attributes.asSequence().map { it.name }
      // 自定义组件的属性
      if (descriptor is WxmlCustomComponentDescriptor) {
        ...
        result.addAllElements(customComponentAttributeDescriptors.filter { desc ->
          ...
          if (WXMLAttributeInsertUtils.isBooleanTypeAttribute(it) && it.defaultValue == "false") {
            LookupElementBuilder.create(it.name).withPriority(100.0)
          } else {
        ...
        val attributes = descriptor.wxmlElementDescription.attributeDescriptors
        // wxml组件属性
        attributes.filter { desc ->
          ...
          result.addAllElements(it.asIterable())
        }
  ...
  private fun addTwoWayBindings(
    result: CompletionResultSet, xmlAttributes: Sequence<String>, attributes: Sequence<String>
  ) {
    // 简易双向绑定
  ...
  private fun addCommonEvents(result: CompletionResultSet, xmlAttributes: Sequence<String>) {
