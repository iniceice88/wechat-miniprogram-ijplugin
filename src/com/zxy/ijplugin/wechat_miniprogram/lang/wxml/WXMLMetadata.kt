...
data class WXMLElementDescription(
  val name: String,
  val attributeDescriptors: Sequence<WXMLPresetElementAttributeDescription> = emptySequence(),
  val events: Sequence<String> = emptySequence(),
...
class WXMLMetadata(private val project: Project) {
  private val elementDescriptors by lazy {
    ResourceUtils.findWXMLMetaDataFile()?.let {
      ...
      // 将psi文件读取为Object
      root?.propertyList?.asSequence()?.mapNotNull { elementJsonProperty ->
        ...
        val events = elementJsonPropertyValue.findStringArrayPropertyValue("events") ?: emptySequence()
        ...
        val attributeDescriptors = findAttributeDescriptors(elementJsonPropertyValue)
  ...
  private fun findAttributeDescriptors(elementJsonPropertyValue: JsonObject): Sequence<WXMLPresetElementAttributeDescription> {
    return elementJsonPropertyValue.findProperty("attributeDescriptors")?.let { attributeJsonProperty ->
      (attributeJsonProperty.value as? JsonArray)?.valueList?.asSequence()?.mapNotNull { attributeJsonObject ->
        if (attributeJsonObject is JsonObject) {
          WXMLPresetElementAttributeDescription(
            ...
            attributeJsonObject.findStringArrayPropertyValue("enums") ?: emptySequence(),
  ...
  companion object {
    fun getElementDescriptions(project: Project): Sequence<WXMLElementDescription> {
    ...
    fun findElementAttributeDescription(xmlTag: XmlTag, attributeName: String): WXMLElementAttributeDescription? {
      return this.findElementDescription(xmlTag)?.let { wxmlElementDescription ->
        wxmlElementDescription.attributeDescriptors.find {
    ...
    val COMMON_ELEMENT_ATTRIBUTE_DESCRIPTORS = arrayOf(
      WXMLElementAttributeDescription("id", sequenceOf(T.STRING)),
      WXMLElementAttributeDescription("class", sequenceOf(T.STRING)),
      WXMLElementAttributeDescription("style", sequenceOf(T.STRING)),
      WXMLElementAttributeDescription("hidden", sequenceOf(T.BOOLEAN), false),
      WXMLElementAttributeDescription("slot", sequenceOf(T.STRING))
    )
...
open class WXMLElementAttributeDescription(
  val key: String,
  val types: Sequence<ValueType> = emptySequence(),
  val default: Any? = null,
  val required: Boolean = false,
  val enums: Sequence<String> = emptySequence(),
  @Suppress("unused")
