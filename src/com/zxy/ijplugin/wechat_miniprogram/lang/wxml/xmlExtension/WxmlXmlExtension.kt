...
private val selfClosingTagNames = arrayOf("input", "textarea", "image", "icon", "import", "include")

class WxmlXmlExtension : DefaultXmlExtension() {

  override fun isAvailable(file: PsiFile): Boolean {
    return file.fileType == WXMLFileType.INSTANCE ||
      (file.project.isQQContext() && file.fileType == QMLFileType.INSTANCE)
  }
  ...
  override fun getAttributeValuePresentation(
    ...
    if (tag === null) return super.getAttributeValuePresentation(tag, attributeName, defaultAttributeQuote)
    ...
    if (attributeDescription.isJsTypeAttribute()) {
      return object : AttributeValuePresentation {
        override fun showAutoPopup(): Boolean {
          return attributeDescription.enums.any()
        }
  ...
  private val wxNamespacePrefixes = setOf("wx", "bind", "catch", "model", "mark", "mut-bind", "generic")

  override fun getPrefixDeclaration(context: XmlTag, namespacePrefix: String?): SchemaPrefix? {
    if (namespacePrefix != null && (
        wxNamespacePrefixes.contains(namespacePrefix)
