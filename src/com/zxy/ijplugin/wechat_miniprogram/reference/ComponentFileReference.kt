...
/**
 * 对于subpackages下的pages，它的路径不是从根目录开始，而是从root指定的路径开始
 * 所以这里需要把root加上
 */
class SubPackageFileReferenceSet(psiElement: PsiElement, val root: String) : ComponentFileReferenceSet(psiElement) {
  override fun computeDefaultContexts(): MutableCollection<PsiFileSystemItem> {
    if (!isWechatMiniProgramContext(this.element.project)) {
      return super.computeDefaultContexts()
    }

    val defaultContexts = mutableListOf<PsiFileSystemItem>()
    val superDefaultContexts = super.computeDefaultContexts()
    for (context in superDefaultContexts) {
      if (context is PsiDirectory) {
        val dir = context.findSubdirectory(root)
        if (dir != null) {
          defaultContexts.add(dir)
          continue
        }
      }
...
class ComponentFileReference(
  ...
  private fun findRelateFilesFromDirectory(dir: PsiDirectory, text: String): List<PsiElementResolveResult> {
    return dir.files.filter {
