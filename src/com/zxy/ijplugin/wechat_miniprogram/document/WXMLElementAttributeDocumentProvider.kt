...
class WXMLElementAttributeDocumentProvider : DocumentationProvider {
  ...
  override fun generateDoc(element: PsiElement, originalElement: PsiElement?): String? {
    if (element !is JsonStringLiteral || element.containingFile.name != "elementDescriptions.json") return null

    val elementName = (element.parent?.parent?.parent?.parent?.parent?.parent as? JsonProperty)?.name ?: return null
    ...
    WXMLMetadata.getElementDescriptions(element.project).find {
      ...
      val contents = mapOf(
        ...
        if (wxmlAttributeDescriptor.enums.any()) {
