...
class WXMLCreateEventHandlerIntentionAction : IntentionAction, PsiElementBaseIntentionAction() {
  ...
  override fun isAvailable(project: Project, editor: Editor?, psiElement: PsiElement): Boolean {
    ...
    // 只要存在Page或Component方法调用且有第一个参数为对象即可
    ...
    val isComponent = ComponentJsUtils.isComponentCall(callExpression.methodExpression?.text)
  ...
  override fun invoke(project: Project, editor: Editor, psiElement: PsiElement) {
    ...
    val property = if (isComponent) {
      val methodsProperty = options.find { it.name == "methods" }
