...
object ComponentJsonUtils {
  ...
  /**
   * 获取一个组件的json文件中注册的所有组件
   */
  fun getUsingComponentItems(jsonPsiFile: JsonFile): Sequence<JsonProperty> {
    val usingComponentsObjectValue = getUsingComponentPropertyValue(jsonPsiFile) ?: return emptySequence()
    return usingComponentsObjectValue.propertyList.asSequence()
  }

  /**
   * 通过componentGenerics组册的组件
   ...
   */
  fun getComponentGenericsItems(jsonPsiFile: JsonFile): Sequence<JsonProperty> {
    val root = jsonPsiFile.topLevelValue as? JsonObject ?: return emptySequence()
    val usingComponentsProperty = root.findProperty("componentGenerics") ?: return emptySequence()
    val itemsList = mutableListOf<JsonProperty>()
    val value = usingComponentsProperty.value as? JsonObject ?: return emptySequence()
    // 有以下两种形式：
    // "componentGenerics": { "selectable": true }
    // "componentGenerics": { "selectable": { "default": "path/to/default/component" } }
    return value.propertyList.asSequence()
  }
