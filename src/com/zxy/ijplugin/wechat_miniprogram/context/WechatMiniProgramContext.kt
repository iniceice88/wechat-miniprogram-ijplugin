...
/**
 * 找到小程序代码根目录
 * 优先从 project.config.json 中读取 miniprogramRoot 字段
 * 如果没有则返回项目根目录
 */
fun findMiniProgramRootDir(project: Project): PsiDirectory? {
  return findMiniProgramRootDirFromProjectConfig(project) ?: findProjectRootDir(project)
}

private fun findMiniProgramRootDirFromProjectConfig(project: Project): PsiDirectory? {
  val jsonFile = findProjectConfigJsonFile(project) ?: return null
  return runReadAction {
    val propMiniprogramRoot = ((jsonFile.children.getOrNull(0) as? JsonObject)?.propertyList?.find {
      it.name == "miniprogramRoot"
    }?.value as? JsonStringLiteral)
    if (propMiniprogramRoot == null) return@runReadAction null

    FileReferenceSet(propMiniprogramRoot).allReferences.last {
      it.text.isNotBlank()
    }?.resolve() as PsiDirectory?
  }
}
...
fun findMiniProgramNpmRootDir(project: Project): PsiDirectory? {
  return findProjectRootDir(project)?.let { rootDir ->
    runReadAction {
      if (rootDir.findFile("package.json") != null && rootDir.findSubdirectory("node_modules") != null) {
        // package.json is existing
        // node_modules is existing
        rootDir.findSubdirectory("miniprogram_npm")
      } else {
        null
      }
    }
